...
delay(30);
double soundLevel = map(soundValue, 0, 1023, 0, 100); // Map the analog value to a 
range of 0 to 100
Blynk.virtualWrite(V2, soundLevel);
Blynk.virtualWrite(V3, lightValue);
if (motionValue == HIGH)
{
Blynk.virtualWrite(V1, "Motion is detected!");
}
else
{
Blynk.virtualWrite(V1, "No motion is detected!");
}
}
BLYNK_CONNECTED()
{
